...
public class CutterController : MonoBehaviour, IMachineController
{
    ...
    void Start()
    {
        this.Inventory = new Queue<Resource>(this.Capacity);
        ...
        this.cutterGUI = PrefabDatabase.Instance.GetPrefab("UI", "MachineBaseCanvas").GetComponent<MachineMasterPanelScript>().CutterCanvas.gameObject;
    }
    ...
    public void AddToInventory(Resource resourceToAdd)
    {
        ...
        if (this.cutterGUI.activeSelf && this.cutterGUI.GetComponent<CutterCanvasScriptNewNew>().CutterController == this)
        {
            this.cutterGUI.GetComponent<CutterCanvasScriptNewNew>().UpdateUI();
        }
    }
